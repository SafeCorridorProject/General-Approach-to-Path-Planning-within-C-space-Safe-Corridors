...
public:
    ...
    ChRobot_6dof_CAD(
        ...
        std::vector<std::string> bodynames = { "Base_UR5_STEP", "Link1_UR5_STEP", "Link2_UR5_STEP", "Link3_UR5_STEP",  "Link4_UR5_STEP_1000515", "Link5_UR5_STEP",/*"Link6_UR5_STEP"*/"End_effector_assembly2"}       ///  NB: if 'base' body is not provided here, robot arm is linked to internal 'ground' instead (fallback)
    );


    // SWITCH TO "End_effector_assembly2" FOR UR5E IN ENV3 (LASER). USE "Link6_UR5_STEP" FOR UR5 IN ENV1 (BLOCKS) AND ENV2 (SHELVES)
